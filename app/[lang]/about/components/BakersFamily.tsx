...
const BakersFamily = async ({ lang }: { lang: Locale }) => {
    ...
    <div className="flex flex-col md:flex-row container mx-auto gap-3 md:gap-2.5 px-4 md:px-0 mb-40">
      ...
      <div className="w-full md:basis-1/2 bg-beige-2 rounded-2xl p-9 md:p-10 md:pl-16 md:pr-40 md:py-40 relative overflow-hidden">
        ...
        <TypographyH2 className="text-left max-w-[12ch] sm:w-full relative z-10">
          <RichText>{dict.pages.about.bakersFamily.title}</RichText>
        </TypographyH2>
        <Separator
        ...
        <TypographyP className="text-lg text-left relative z-10">
          <RichText>{dict.pages.about.bakersFamily.description}</RichText>
        </TypographyP>
